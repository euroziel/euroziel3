...
function StepCard({ step, isActive, dark }: { step: typeof steps[0]; isActive: boolean; dark: boolean }) {
  ...
  return (
    <motion.div
      ...
      className="w-screen flex-shrink-0 px-6 mobile-m:px-8 laptop:px-24 4k:px-32 flex justify-center"
    >
      <div
...
export default function Journey({ theme }: JourneyProps) {
  ...
  const idx = Math.round(v * (numSteps - 1));
  setActiveIndex(Math.min(numSteps - 1, Math.max(0, idx)));
});

  const numSteps = steps.length; // 6
  const breakpoints = steps.map((_, i) => i / (numSteps - 1)); // [0, .2, .4, .6, .8, 1]
  const vwOffsets = steps.map((_, i) => `-${i * 100}vw`);        // ["0vw","-100vw",...,"-500vw"]

  const translateX = useTransform(smoothProgress, breakpoints, vwOffsets);

  const progressWidth = useTransform(
    smoothProgress,
    breakpoints,
    steps.map((_, i) => `${Math.max(3, (i / (numSteps - 1)) * 100)}%`)
  );

  const charLeft = useTransform(smoothProgress, breakpoints, steps.map((_, i) => `${(i / (numSteps - 1)) * 100}%`));
  return (
    <>
      {/* 
      ...
      */}
      ...
      >
        ...
        */}
        <div
          className={`sticky top-0 h-screen flex flex-col ${dark ? "bg-[#060814]" : "bg-white"
            }`}
          style={{
        ...
        >
          ...
          <div className="pt-20 pb-4 text-center px-4 flex-shrink-0">
            <motion.h2
              ...
              viewport={{ once: true }}
              className={`font-bold text-3xl mobile-m:text-4xl laptop:text-5xl ${dark ? "text-slate-100" : "text-slate-900"
                }`}
            >
              Six Steps To{" "}
                ...
                {steps.map((s, idx) => {
                  ...
                  return (
                    <div key={s.number} className="flex flex-col items-center">
                      ...
                      <span
                        className={`text-[8px] mobile-m:text-[9px] laptop:text-[11px] font-bold uppercase tracking-wider mt-1.5 transition-colors duration-200 ${active
          ...
          <div className="flex-1 flex items-center overflow-hidden">
            <motion.div
              style={{ x: translateX }}
              className="flex laptop:h-[420px] 4k:h-[480px] items-center"
            >
              {steps.map((step, i) => (
                <StepCard key={step.number} step={step} isActive={activeIndex === i} dark={dark} />
              ))}
            </motion.div>
          </div>
