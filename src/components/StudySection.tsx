    ...
    <div className="space-y-24 md:space-y-36 pb-20">
      ...
      <section className="max-w-7xl mx-auto px-4 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, idx) => (
            <div 
              key={idx}
              className={`p-6 rounded-sm border text-left space-y-3 shadow-premium hover:border-gold hover:shadow-premium transition-all relative overflow-hidden group border-b-2 border-b-navy ${
        ...
        <div className="max-w-7xl mx-auto space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {specialisations.map((spec, idx) => (
              <div 
                key={idx}
                className={`p-6 rounded-sm border text-left space-y-4 shadow-premium hover:border-gold transition-all duration-300 ${
                  dark ? 'border-slate-800 bg-slate-950' : 'border-slate-200/50 bg-white'
                }`}
              >
                <div className={`w-10 h-10 rounded-sm border flex items-center justify-center shrink-0 text-navy ${
                  dark ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200/40'
                }`}>
                  {spec.icon}
                </div>
                <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6 space-y-6 text-left">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className={`p-4 rounded-sm border shadow-sm border-b-2 border-b-gold ${
                dark ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200/60 bg-white/50'
              }`}>
              ...
              <div className={`p-4 rounded-sm border shadow-sm border-b-2 border-b-navy ${
                dark ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200/60 bg-white/50'
              }`}>
          ...
          <div className="lg:col-span-6">
            <div className={`p-6 md:p-8 rounded-sm border shadow-premium ${
              dark ? 'border-slate-800 bg-slate-950/80' : 'border-slate-200/50 bg-white'
            }`}>
              ...
              <div className="flex flex-wrap gap-2 justify-start">
                {brandLogs.map((brand, bIdx) => (
                  <span 
                    key={bIdx}
                    className={`px-4 py-2 border rounded-sm text-xs font-bold font-sans hover:text-[#e5a800] transition-colors ${
                      dark ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-250'
                    }`}
                  >
                    {brand}
                  </span>
      ...
      <section className={`py-20 px-4 border-y ${
        ...
        <div className="max-w-4xl mx-auto text-left space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className={`rounded-sm border p-6 md:p-8 space-y-4 shadow-premium border-b-4 border-b-rose-500 ${
            ...
            <div className={`rounded-sm border p-6 md:p-8 space-y-4 shadow-premium border-b-4 border-b-emerald-500 ${
              dark ? 'border-slate-800 bg-slate-950' : 'border-slate-200 bg-white'
            }`}>
              <div className="flex items-center justify-between">
      ...
      <section className="max-w-4xl mx-auto px-4 text-center">
        <div className={`p-8 md:p-12 rounded-sm border shadow-premium space-y-6 border-b-4 border-b-gold ${
          dark ? 'border-slate-800 bg-slate-950 text-white' : 'border-slate-200 bg-white text-slate-800'
        }`}>
