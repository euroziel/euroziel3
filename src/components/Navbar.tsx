...
export default function Navbar({ currentTab, onTabChange, theme, onThemeToggle, onOpenConsultation }: NavbarProps) {
  ...
  return (
    <nav className={`fixed top-4 left-1/2 -translate-x-1/2 z-40 border transition-all rounded-full duration-300 backdrop-blur-md w-max ${
      theme === 'dark'
      ...
      <div className="px-3 laptop:px-4">
        <div className="flex items-center gap-1 h-14 laptop:h-16">
          ...
          {/* Logo */}
          <div className="flex-shrink-0 cursor-pointer pr-2 border-r mr-1 ${
            ...
            {menuItems.map(({ id, label, Icon }) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-semibold tracking-wide whitespace-nowrap transition-all duration-200 cursor-pointer ${
                    isActive
                      ? theme === 'dark'
          ...
          <div className="flex lg:hidden items-center gap-2 ml-2">
            <button
              ...
              className={`p-1.5 rounded-full border transition-colors cursor-pointer ${
                theme === 'dark' ? 'border-slate-700 bg-slate-900 text-gold' : 'border-slate-200 bg-slate-50 text-slate-600'
              }`}
            >
              {theme === 'dark'
                ? <Sun className="w-3.5 h-3.5" />
      ...
      {/* Mobile Drawer */}
      {isOpen && (
        <div className={`lg:hidden border-t absolute top-14 left-0 right-0 shadow-xl z-50 rounded-b-2xl overflow-hidden ${
          ...
          <div className="px-4 pt-3 pb-5 space-y-3">
            <div className="flex flex-col space-y-0.5">
              {menuItems.map(({ id, label, Icon }) => (
                <button
                  key={id}
                  onClick={() => handleNavClick(id)}
                  className={`w-full text-left py-2.5 px-3 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-colors cursor-pointer flex items-center gap-3 ${
                    currentTab === id
                      ? 'bg-navy/10 text-navy'
            ...
            <div className="pt-1">
              <button
                ...
                className={`w-full py-2.5 rounded-xl font-bold text-[11px] uppercase tracking-wider text-center border transition-all flex items-center justify-center gap-2 ${
